    ...
    <AnimationOnScroll animateIn="animate__slideInUp" animateOut="animate__slideInUp">
      <div id="experience" className={`${S.Container}`} >
        ...
        <div className=" d-flex align-items-start">
            <div className="d-flex align-items-start" id="nav-tab" role="tablist">
            ...
            <div className="tab-content justify-content-center" id="nav-tabContent">
              {Experience.map((item, key) => {
                return <div className={key === 0 ? "tab-pane fade show active" : "tab-pane fade"}
                  ...
                  tabindex="0"
                >
                  <h1 className={S.PostTitle} > {item.post} <span>@ {item.company} </span></h1>
                  <h3 className={S.Date}> {item.date} </h3>
                  <ul> {item.discription.map(((desc, index) => {
                    return <li className={S.ListDescription} key={index} > {desc} </li>
                  }))} </ul>
                  {item.link && <a href={item.link} className={S.VisitWebsite} target="_blank" >Visit Web site</a>}
                </div>
              })}
            </div>
          </div>
